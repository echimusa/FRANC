...
#"""convertrecfile.pl converts between CDF and PDF style recombination map files, and can take a wide varienty of map formats and convert them into a 
#suitable format for ChromoPainter. For example, the HapMap B37 data obtained from nih can be processed with "convertrecfile.pl -M hapmap",
#but other formats are supported."""
...
print "Copyright (C) 2013 Daniel Lawson (dan.lawson\@bristol.ac.uk) licenced under GPLv3\n";
...
print "                 This is the output mode of chromopainter (note: the rate is Morgans per base).\n";
...
print " -f <a,b>:       The fields to be used as the position, and the recombination rate, with the first column indexed as 1 (Default: 1,2).\n";
...
## Extract options
my %Options;
my $continue = getopts('I:N:U:M:T:t:d:D:i:hsHAv', \%Options);
